...
//Complexity
//        Time        O(n) — one pass to build maps + O(n) DFS per root, total O(n)
...
class LargestTree
{
    ...
    public static Integer largestTree(final Map<Integer, Integer> immediateParent) {
        // Step 1: Build parent -> list of children map & collect all nodes
        ...
        // Step 2: Roots are nodes that never appear as a child
        Set<Integer> roots = new HashSet<>(allNodes);
        roots.removeAll(immediateParent.keySet());

        // Step 3: For each root, compute tree size; track largest (smallest id on tie)
